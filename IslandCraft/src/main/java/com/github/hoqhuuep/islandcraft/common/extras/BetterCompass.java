...
public class BetterCompass {
	...
	public void onNextTarget(final ICPlayer player) {
		if (!player.getWorld().isNormalWorld()) {
			...
			player.info("Compass now pointing to " + ChatColor.MAGIC
					+ "nowhere");
			return;
		}
	...
	private void setTarget(final ICPlayer player,
		...
		case BED:
			...
			if (bedLocation == null
					|| !player.getServer()
							.findOnlineWorld(bedLocation.getWorld())
							.isNormalWorld()) {
		...
		case DEATH_POINT:
			...
			if (deathPoint == null
					|| !player.getServer()
							.findOnlineWorld(deathPoint.getWorld())
							.isNormalWorld()) {
