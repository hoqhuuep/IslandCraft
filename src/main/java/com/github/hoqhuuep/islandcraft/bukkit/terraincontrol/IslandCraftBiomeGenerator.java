...
public class IslandCraftBiomeGenerator extends BiomeGenerator {
	...
	public IslandCraftBiomeGenerator(final LocalWorld world,
			final BiomeCache cache) {
		super(world, cache);
		this.seed = world.getSeed();
	}

	public final int[] calculate(final int xStart, final int zStart,
			final int xSize, final int zSize) {
		final int[] array = new int[xSize * zSize];
		for (int x = 0; x < xSize; ++x) {
			for (int z = 0; z < zSize; ++z) {
				array[x + z * xSize] = generator.biomeAt(seed, x + xStart, z
						+ zStart);
			}
		}
		return array;
	...
	public final int[] calculateUnZoomed(final int xStart, final int zStart,
			final int xSize, final int zSize) {
		final int[] array = new int[xSize * zSize];
		for (int x = 0; x < xSize; ++x) {
			for (int z = 0; z < zSize; ++z) {
				array[x + z * xSize] = generator.biomeAt(seed,
						(x + xStart) * 4, (z + zStart) * 4);
			}
		}
		return array;
	}
	...
	@Override
	public final int[] getBiomes(int[] biomeArray, final int x, final int z,
			final int x_size, final int z_size) {
	...
	@Override
	public final int[] getBiomesUnZoomed(int[] biomeArray, final int x,
	...
	@Override
	public final float[] getRainfall(float[] rainfallArray, final int x,
			final int z, final int x_size, final int z_size) {
		...
		for (int i = 0; i < x_size * z_size; i++) {
			float rainfall = worldConfig.biomeConfigs[biomeArray[i]]
					.getWetness() / 65536.0F;
	...
	@Override
	public final float[] getTemperatures(float[] temperatureArray, final int x,
			final int z, final int x_size, final int z_size) {
		if (temperatureArray == null
				|| temperatureArray.length < x_size * z_size) {
			temperatureArray = new float[x_size * z_size];
		}
		final int[] biomeArray = calculate(x, z, x_size, z_size);
		for (int i = 0; i < x_size * z_size; i++) {
			float temperature = worldConfig.biomeConfigs[biomeArray[i]]
					.getTemperature() / 65536.0F;
