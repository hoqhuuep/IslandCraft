...
public class PerlinNoise {
	...
	private static BufferedImage circle() {
		if (circleCache == null) {
			circleCache = new BufferedImage(64, 64,
					BufferedImage.TYPE_4BYTE_ABGR_PRE);
			for (int z = 0; z < 64; ++z) {
				for (int x = 0; x < 64; ++x) {
					int dx = 32 - x;
					int dz = 32 - z;
					int d = Math.max(0x70 - ((dx * dx * dx * dx + dz * dz * dz
							* dz) * 0x70 / (32 * 32 * 32 * 16)), 0);
	...
	private static BufferedImage perlinNoise(final Random random) {
		...
		g.dispose();
		BufferedImage result2 = new BufferedImage(256, 256,
	...
	private static BufferedImage whiteNoise(final int width, final int height,
			final Random random) {
