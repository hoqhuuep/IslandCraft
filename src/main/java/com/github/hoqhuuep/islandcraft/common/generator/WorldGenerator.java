...
public final class WorldGenerator {
	...
	public int biomeAt(final int x, final int z) {
		...
		final int rz = Geometry.mod(zz, islandSeparation);
		if (rz >= islandSize) {
			return oceanBiome;
		}
		final int xx = x + (islandSize >> 1);
		final int row = Geometry.div(zz, islandSeparation);
		final int xxx;
		if (0 == row % 2) {
			xxx = xx;
		} else {
		...
		final int rx = Geometry.mod(xxx, islandSeparation);
		if (rx >= islandSize) {
			return oceanBiome;
		}
		final int cz = row * islandSeparation;
		final int col = Geometry.div(xxx, islandSeparation);
		final int cx;
		if (0 == row % 2) {
