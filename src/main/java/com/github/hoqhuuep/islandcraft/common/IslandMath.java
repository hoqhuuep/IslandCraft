...
public class IslandMath {
	...
	public ICLocation islandAt(ICLocation location) {
		...
		return (rx >= islandSize || rz >= islandSize) ? null : new ICLocation(
				location.getWorld(), cx, cz);
	}

	public ICRegion visibleRegion(ICLocation island) {
		final int visRad = config.getIslandSize() * 8;
		return new ICRegion(island.add(-visRad, -visRad), island.add(visRad,
				visRad));
	}

	public ICRegion protectedRegion(ICLocation island) {
		final int proRad = config.getIslandSize() * 8 + config.getIslandGap()
				* 16;
		return new ICRegion(island.add(-proRad, -proRad), island.add(proRad,
				proRad));
	}

	public long originalSeed(ICLocation island) {
		final long worldSeed = server.findOnlineWorld(island.getWorld())
				.getSeed();
